<script lang="ts">
	...
	let {
		members = [],
		votedIds = []
	}: { members?: { id: string; name?: string; email?: string }[]; votedIds?: string[] } = $props();
	...
	<div class="flex flex-col items-center">
		<div class="relative h-40 w-40">
			...
			<div class="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
				<span class="text-2xl leading-none font-bold text-brand-dark">
					{votedIds.length}<span class="text-gray-400">/{members.length}</span>
				</span>
				<span class="mt-1 text-xs text-gray-500">voted</span>
			</div>
		</div>
		<p class="mt-4 flex items-center gap-4 text-xs text-gray-500">
